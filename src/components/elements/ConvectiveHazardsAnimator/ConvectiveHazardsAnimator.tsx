...
import { HAZARD_TYPE_HYDROLOGICAL_ID, HAZARD_TYPE_SEVERE_ID, HAZARD_TYPE_TORNADO_ID } from '@/data/hazardMapVars'
import { DATA_TEXT_HAZARDS_MAP_DETAILS_SLIDEOUT } from '@/data/vars'
import { useRootStore } from '@/store/useRootStore'
import { mapZoomState } from '@/types/general'
import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import styles from './ConvectiveHazardsAnimator.module.scss'

// Convective hazard types to filter for (includes hydrological since flash floods are often convective)
const CONVECTIVE_HAZARD_TYPES = [HAZARD_TYPE_TORNADO_ID, HAZARD_TYPE_SEVERE_ID, HAZARD_TYPE_HYDROLOGICAL_ID]
...
/**
 * Check if a hazard is a convective type (TORNADO or SEVERE)
 */
const isConvectiveHazard = (alert: any): boolean => {
	const hazardType = alert?.hazardInfo?.type?.type || alert?.hazardType
	return CONVECTIVE_HAZARD_TYPES.includes(hazardType)
}

/**
 * Filter alerts to only include convective hazards
 */
const filterConvectiveAlerts = (alerts: any[]): any[] => {
	return alerts.filter(isConvectiveHazard)
}

interface ConvectiveHazardsAnimatorProps {
...
/**
 * ConvectiveHazardsAnimator - Deck.gl map-based convective hazards visualization
 *
 * This component wraps the Animator with map mode to display
 * convective weather hazards (Tornado and Severe Thunderstorm warnings/watches).
 * It filters the hazard data to only show convective-related alerts.
 */
export const ConvectiveHazardsAnimator = ({ alerts, allCoastalRegions }: ConvectiveHazardsAnimatorProps) => {
	...
	useEffect(() => {
		if (selectedRegion && alerts) {
			...
			if (regionName && alerts[regionName]) {
				...
				// Filter to only include locations with convective alerts
				...
				Object.entries(regionAlerts).forEach(([locationId, locationData]: [string, any]) => {
					...
					const convectiveAlerts = filterConvectiveAlerts(locationAlerts)

					if (convectiveAlerts.length > 0) {
						filteredAlerts[locationId] = {
							...locationData,
							alerts: convectiveAlerts,
						}
					}
				})
	...
	// Show empty state if no convective hazards for this region
	// Note: This is expected behavior - convective hazards (tornado/severe thunderstorm/hydrological)
	// may not always be active, especially outside of spring/summer storm season
	if (frames.length === 0) {
		return (
			<div className={styles.convectiveHazardsAnimator}>
				<div className={styles.emptyState}>
					<p>No active convective hazards</p>
					<p className={styles.emptyStateSubtext}>
						No current Tornado, Severe Thunderstorm, or Flood warnings/watches. Check back during active severe weather events.
					</p>
				</div>
			</div>
		)
