...
import styles from './MarineHazardsAnimator.module.scss'
 ...
 */
const isMarineHazard = (alert: any): boolean => {
	const hazardType = alert?.hazardInfo?.type?.type || alert?.hazardType
	return MARINE_HAZARD_TYPES.includes(hazardType)
}

/**
 * Filter alerts to only include marine hazards
 */
const filterMarineAlerts = (alerts: any[]): any[] => {
	return alerts.filter(isMarineHazard)
}

interface MarineHazardsAnimatorProps {
...
/**
 * MarineHazardsAnimator - Deck.gl map-based marine hazards visualization
 *
 * This component wraps the Animator with map mode to display
 * marine hazards (Tsunami, Storm, Gale, High Surf, Coastal Flood, Small Craft warnings/watches/advisories).
 * It filters the hazard data to only show marine-related alerts.
 */
export const MarineHazardsAnimator = ({ alerts, allCoastalRegions }: MarineHazardsAnimatorProps) => {
	...
	useEffect(() => {
		if (selectedRegion && alerts) {
			...
			if (regionName && alerts[regionName]) {
				...
				// Filter to only include locations with marine alerts
				...
				Object.entries(regionAlerts).forEach(([locationId, locationData]: [string, any]) => {
					...
					const marineAlerts = filterMarineAlerts(locationAlerts)
	...
	// Show empty state if no marine hazards for this region
	if (frames.length === 0) {
		return (
			<div className={styles.marineHazardsAnimator}>
