...
import { HAZARD_TYPE_HYDROLOGICAL_ID } from '@/data/hazardMapVars'
import { DATA_TEXT_HAZARDS_MAP_DETAILS_SLIDEOUT } from '@/data/vars'
import { useRootStore } from '@/store/useRootStore'
import { mapZoomState } from '@/types/general'
import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import styles from './HydrologicalHazardsAnimator.module.scss'

// Hydrological hazard type to filter for
const HYDROLOGICAL_HAZARD_TYPES = [HAZARD_TYPE_HYDROLOGICAL_ID]
...
/**
 * Check if a hazard is a hydrological type
 */
const isHydrologicalHazard = (alert: any): boolean => {
	const hazardType = alert?.hazardInfo?.type?.type || alert?.hazardType
	return HYDROLOGICAL_HAZARD_TYPES.includes(hazardType)
}

/**
 * Filter alerts to only include hydrological hazards
 */
const filterHydrologicalAlerts = (alerts: any[]): any[] => {
	return alerts.filter(isHydrologicalHazard)
}

interface HydrologicalHazardsAnimatorProps {
...
/**
 * HydrologicalHazardsAnimator - Deck.gl map-based hydrological hazards visualization
 *
 * This component wraps the Animator with map mode to display
 * hydrological hazards (Flash Flood, Flood, Coastal Flood warnings/watches/advisories).
 * It filters the hazard data to only show hydro-related alerts.
 */
export const HydrologicalHazardsAnimator = ({ alerts, allCoastalRegions }: HydrologicalHazardsAnimatorProps) => {
	...
	useEffect(() => {
		if (selectedRegion && alerts) {
			...
			if (regionName && alerts[regionName]) {
				...
				// Filter to only include locations with hydrological alerts
				...
				Object.entries(regionAlerts).forEach(([locationId, locationData]: [string, any]) => {
					...
					const hydrologicalAlerts = filterHydrologicalAlerts(locationAlerts)

					if (hydrologicalAlerts.length > 0) {
						filteredAlerts[locationId] = {
							...locationData,
							alerts: hydrologicalAlerts,
						}
					}
				})
	...
	useEffect(() => {
		...
		const frame: MapFrame = {
			id: 'current-hydrological-hazards',
			timestamp: new Date(),
			data: countyGeoJSON,
	...
	// Show empty state if no hydrological hazards for this region
	if (frames.length === 0) {
		return (
			<div className={styles.hydrologicalHazardsAnimator}>
				<div className={styles.emptyState}>
					<p>No active hydrological hazards</p>
					<p className={styles.emptyStateSubtext}>
						No current Flash Flood, Flood, or Coastal Flood warnings/watches/advisories. Check back during active hydrological weather
						events.
					</p>
				</div>
			</div>
