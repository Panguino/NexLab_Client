...
import styles from './FireHazardsAnimator.module.scss'
 ...
 */
const isFireHazard = (alert: any): boolean => {
	const hazardType = alert?.hazardInfo?.type?.type || alert?.hazardType
	return FIRE_HAZARD_TYPES.includes(hazardType)
}

/**
 * Filter alerts to only include fire hazards
 */
const filterFireAlerts = (alerts: any[]): any[] => {
	return alerts.filter(isFireHazard)
}

interface FireHazardsAnimatorProps {
...
/**
 * FireHazardsAnimator - Deck.gl map-based fire hazards visualization
 *
 * This component wraps the Animator with map mode to display
 * fire hazards (Fire Warning, Red Flag Warning, Fire Weather Watch).
 * It filters the hazard data to only show fire-related alerts.
 */
export const FireHazardsAnimator = ({ alerts, allCoastalRegions }: FireHazardsAnimatorProps) => {
	...
	useEffect(() => {
		if (selectedRegion && alerts) {
			...
			if (regionName && alerts[regionName]) {
				...
				// Filter to only include locations with fire alerts
				...
				Object.entries(regionAlerts).forEach(([locationId, locationData]: [string, any]) => {
					...
					const fireAlerts = filterFireAlerts(locationAlerts)
	...
	// Show empty state if no fire hazards for this region
	if (frames.length === 0) {
		return (
			<div className={styles.fireHazardsAnimator}>
