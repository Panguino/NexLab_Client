...
import styles from './ConvectiveHazardsTable.module.scss'

// Convective hazard types to filter for (matches ConvectiveHazardsAnimator)
const CONVECTIVE_HAZARD_TYPES = [HAZARD_TYPE_TORNADO_ID, HAZARD_TYPE_SEVERE_ID, HAZARD_TYPE_HYDROLOGICAL_ID]

/**
 * Check if a hazard is a convective type
 */
const isConvectiveHazard = (hazardType: string): boolean => {
	return CONVECTIVE_HAZARD_TYPES.includes(hazardType)
}

const ConvectiveHazardsTable = () => {
	...
	// Build row data filtered for convective hazards only
	const rowData: any[] = []
	Object.keys(regionHazards).forEach((key) => {
		...
		alerts.forEach((alert: any) => {
			...
			// Only include convective hazards
			if (hazardType && isConvectiveHazard(hazardType)) {
				rowData.push({
					STATE,
					COUNTYNAME,
					NAME,
	...
	// Show empty state if no convective hazards
	if (rowData.length === 0) {
		return (
			<div className={styles.convectiveHazardsTable}>
	...
	return (
		<div className={`${styles.convectiveHazardsTable} ag-theme-material-dark`}>
...
export default ConvectiveHazardsTable
