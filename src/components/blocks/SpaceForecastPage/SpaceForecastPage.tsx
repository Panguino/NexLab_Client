...
import { SPACE_TEXT_PRODUCTS } from '@/data/text/space/products'
import { getSpaceTextProductHistory } from '@/util/dataCalls/text/query-space'
import { useRouter } from 'next/navigation'
import { useEffect, useMemo, useState } from 'react'
import styles from './SpaceForecastPage.module.scss'

interface SpaceForecastPageProps {
	officeId: string
	productId: string
	validTime: string
}

export const SpaceForecastPage = ({ officeId, productId, validTime }: SpaceForecastPageProps) => {
	...
	// Space product keys have format like: FOUS04_3HRCON, WOXX04_ALTTP2, AXXX80_DAYEVT, etc.
	const productType = useMemo(() => {
		// Extract the product type code from after the underscore
		const match = productId.match(/_([A-Z0-9]{3})/)
		if (match) {
			const code = match[1]
			// Map to our product IDs
			if (code === '3HR') return '3HR'
			if (code === 'ALT') return 'ALT'
			if (code === 'WAR') return 'WAR'
			if (code === 'WAT') return 'WAT'
			if (code === 'CUR') return 'CUR'
			if (code === 'ADV') return 'ADV'
			if (code === 'DAY') return 'DAY'
			if (code === 'WEK') return 'WEK'
		}
		return null
	}, [productId])

	const pageTitle = productType ? SPACE_TEXT_PRODUCTS[productType]?.name : 'Space Weather Product'
	const spaceBasePath = '/weather-data/text-hazards-outlooks/swpc-space-weather'
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			try {
				const data = await getSpaceTextProductHistory(officeId, productId)
				if (data) {
					setTextData(data)
				}
			} catch (error) {
	...
	useEffect(() => {
		const fetchTextContent = async () => {
			...
			try {
				...
				// Space products return HTML with text in <pre> tag
	...
	const handleValidtimeChange = (newValidtimeId: string) => {
		const newPath = `${spaceBasePath}/${officeId}/${productId}/${newValidtimeId}`
		router.push(newPath)
	}
	...
	if (isLoadingData) {
		return (
			<ScrollArea>
				<div className={styles.spaceForecastPage}>
