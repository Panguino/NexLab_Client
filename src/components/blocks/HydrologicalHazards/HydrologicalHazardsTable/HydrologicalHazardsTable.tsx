...
import styles from './HydrologicalHazardsTable.module.scss'
 ...
 */
const isHydrologicalHazard = (hazardType: string): boolean => {
	return HYDROLOGICAL_HAZARD_TYPES.includes(hazardType)
}

const HydrologicalHazardsTable = () => {
	...
	// Build row data filtered for hydrological hazards only
	const rowData: any[] = []
	Object.keys(regionHazards).forEach((key) => {
		...
		alerts.forEach((alert: any) => {
			...
			// Only include hydrological hazards
			if (hazardType && isHydrologicalHazard(hazardType)) {
				rowData.push({
					STATE,
					COUNTYNAME,
					NAME,
	...
	// Show empty state if no hydrological hazards
	if (rowData.length === 0) {
		return (
			<div className={styles.hydrologicalHazardsTable}>
	...
	return (
		<div className={`${styles.hydrologicalHazardsTable} ag-theme-material-dark`}>
...
export default HydrologicalHazardsTable
