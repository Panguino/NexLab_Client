...
interface WPCFrontsAnimatorSettingsProps {
	refreshData: () => void
}

const WPCFrontsAnimatorSettings = ({ refreshData }: WPCFrontsAnimatorSettingsProps) => {
	...
	const refreshWPCFrontsData = async () => {
		if (forecastDataRefreshActive && refreshData) {
			refreshData()
		}
	}
	const { timeRemaining } = useIntervalWithCountdown(refreshWPCFrontsData, forecastDataRefreshInterval * 60 * 1000)
...
export default WPCFrontsAnimatorSettings
