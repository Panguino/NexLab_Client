...
interface HydroAnalysisAnimatorSettingsProps {
	refreshData: () => void
}

const HydroAnalysisAnimatorSettings = ({ refreshData }: HydroAnalysisAnimatorSettingsProps) => {
	...
	const refreshHydroData = async () => {
		if (analysisDataRefreshActive && refreshData) {
			refreshData()
		}
	}
	const { timeRemaining } = useIntervalWithCountdown(refreshHydroData, analysisDataRefreshInterval * 60 * 1000)
...
export default HydroAnalysisAnimatorSettings
