...
import { getClimateTextHistory } from '@/util/dataCalls/text/query-climate'
import { useRouter } from 'next/navigation'
import { useEffect, useMemo, useState } from 'react'
import styles from './ClimateTextPage.module.scss'

interface ClimateTextPageProps {
	productId: string
	validTime: string
}

export const ClimateTextPage = ({ productId, validTime }: ClimateTextPageProps) => {
	...
	const pageTitle = useMemo(() => {
		return product ? product.label : 'Climate Product'
	}, [product])
	...
	useEffect(() => {
		const fetchTextContent = async () => {
			if (!textData || !actualValidtimeId || !product?.prodQueryString) {
				setDisplayText(null)
			...
			try {
				...
				// Climate products return HTML with text in <pre> tag
		...
		fetchTextContent()
	}, [textData, actualValidtimeId, product?.prodQueryString])

	// Handle validtime selection change
	const handleValidtimeChange = (newValidtimeId: string) => {
		const newPath = `${climateBasePath}/text/${productId}/${newValidtimeId}`
		router.push(newPath)
	}
	...
	if (isLoadingData) {
		return (
			<ScrollArea>
				<div className={styles.climateTextPage}>
					<div className={styles.titleSection}>
						<span className={styles.category}>Climate Discussion</span>
						<h1>{pageTitle}</h1>
					</div>
