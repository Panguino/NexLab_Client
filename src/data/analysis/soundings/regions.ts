...
export const ALL_SOUNDING_REGIONS: { [key: string]: SoundingRegion } = {
	[SOUNDING_REGION_CONUS]: {
		...
		rotate: [98, -40],
		scale: 2,
		sites: Object.keys(CONUS_SOUNDING_SITES).map((key) => key),
	},
	[SOUNDING_REGION_ALASKA]: {
		label: 'Alaska',
		rotate: [153, -63],
		scale: 3,
		sites: Object.keys(ALASKA_SOUNDING_SITES).map((key) => key),
	},
	[SOUNDING_REGION_CANADA]: {
		label: 'Canada',
		rotate: [100.88, -60.02],
		scale: 2,
		sites: Object.keys(CANADA_SOUNDING_SITES).map((key) => key),
	},
	[SOUNDING_REGION_MEXICO]: {
		label: 'Mexico & Caribbean',
		rotate: [87.78, -22.92],
		scale: 2,
		sites: Object.keys(MEXICO_SOUNDING_SITES).map((key) => key),
	},
}
